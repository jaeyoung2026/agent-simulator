#!/usr/bin/env python3
"""
Premium + Distributed Strategy Simulation - OPTIMIZED MODEL ALLOCATION
Thesis-First 4-Stage Pattern with Cost-Efficient Model Distribution

OPTIMIZATION STRATEGY:
- Steps 1-3: Flash (Sonnet) - 75% of processing
- Step 4 only: Pro (Opus) - 25% of processing
- Expected savings: ~60-70% vs full Pro

Model Allocation:
- Step 1 (Flash/Sonnet): Thesis extraction + Self-Critique + Deep image analysis
- Step 2 (Flash/Sonnet): Thesis-aware cluster analysis (parallel)
- Step 3 (Flash/Sonnet): Consistency validation + Reverse outline
- Step 4 (Pro/Opus): Paper quality verification ONLY
"""
...
# ============================================================================
# STEP 1: Thesis Extraction + Self-Critique (Flash/Sonnet)
# ============================================================================
...
def step1_extract_thesis(samples: List[Dict]) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    input_tokens = len(samples) * int(avg_content_length * 2.5)  # Content + prompt
    input_tokens += total_images * 800  # Image tokens (lower for Flash)
    output_tokens = len(semantic_units) * 250  # Unit extractions
...
def step2_cluster_analysis(step1_results: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    input_tokens = len(cluster_analyses) * 1200  # Context per cluster
    output_tokens = len(cluster_analyses) * 600  # Analysis per cluster
...
def step4_quality_verification(
    ...
    # Writing Principles Evaluation (Premium feature - Pro depth)
    writing_principles = {
        "thesis_clarity": {
    ...
    # Token usage for Step 4 (Pro/Opus - COMPREHENSIVE REVIEW)
    # This is the only step using expensive Pro model
    input_tokens = 10000  # All previous results + comprehensive context
    output_tokens = 5000  # Detailed gap analysis + writing principles + recommendations
...
def run_premium_distributed_optimized(samples: List[Dict]) -> Dict[str, Any]:
    """
    Run the complete 4-stage Premium + Distributed simulation with optimized model allocation
    """
    print("\n" + "="*70)
    print("PREMIUM + DISTRIBUTED STRATEGY SIMULATION - OPTIMIZED")
    ...
    print("  Step 4: Pro (Opus) - FINAL QUALITY VERIFICATION ONLY")
    ...
    total_tokens_all_steps = flash_input_tokens + flash_output_tokens + pro_input_tokens + pro_output_tokens
    full_pro_cost = (
        (flash_input_tokens + pro_input_tokens) / 1_000_000 * PRO_INPUT_COST +
        (flash_output_tokens + pro_output_tokens) / 1_000_000 * PRO_OUTPUT_COST +
        image_cost
    )
    ...
    cost_estimate = {
        ...
        "optimization_analysis": {
            "optimized_cost": round(total_cost, 4),
            "full_pro_cost": round(full_pro_cost, 4),
    ...
    final_results = {
        "condition": "Premium (Distributed - Optimized)",
        ...
        "quality_metrics": {
            ...
            "consistency_score": step3_results['consistency_score'],
            "alignment_rate": step3_results['alignment_rate'],
            "avg_self_critique": step1_results['self_critique_summary']['avg_score'],
            "high_severity_gaps": step4_results['summary']['high_severity_gaps'],
...
# Run simulation
results = run_premium_distributed_optimized(samples)

# Print summary
print("\n" + "="*70)
print("SIMULATION COMPLETE - OPTIMIZED MODEL ALLOCATION")
print("="*70)
...
print("="*70)
print(f"\nModel Distribution:")
print(f"  Flash (Sonnet) - Steps 1-3: {results['cost_optimization']['flash_ratio']} of tokens")
print(f"  Pro (Opus) - Step 4 only: {results['cost_optimization']['pro_ratio']} of tokens")
...
print(f"\nSavings vs Full Pro:")
print(f"  Optimized:    ${results['cost_estimate']['optimization_analysis']['optimized_cost']:.4f}")
print(f"  Full Pro:     ${results['cost_estimate']['optimization_analysis']['full_pro_cost']:.4f}")
print(f"  Savings:      ${results['cost_estimate']['optimization_analysis']['cost_savings']:.4f}")
print(f"  Percentage:   {results['cost_estimate']['optimization_analysis']['savings_percentage']:.1f}%")

# Save results
output_path = '/Users/jaeyoungkang/workspace/moonwrite-simulator/working-contexts/experiments/2026-01-31-exp2/exp5-premium-distributed-optimized.json'
